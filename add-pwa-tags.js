// Script Node.js để thêm PWA meta tags vào các file HTML
const fs = require('fs');
const path = require('path');

const pwaMeta = `  <!-- ===== PWA META TAGS HOÀN CHỈNH ===== -->
  <!-- Viewport chuẩn cho PWA -->
...
const orientationWarning = `  <!-- Orientation warning HTML -->
...
function addPWAToFile(filename) {
  try {
    let content = fs.readFileSync(filename, 'utf8');
    
    // Thêm PWA meta tags sau charset
    if (content.includes('<meta charset') && !content.includes('PWA META TAGS')) {
      const charsetMatch = content.match(/(<meta charset[^>]*>)/);
      if (charsetMatch) {
        content = content.replace(charsetMatch[1], charsetMatch[1] + '\n' + pwaMeta);
      }
    }
    
    // Thêm JavaScript trước </head>
    if (content.includes('</head>') && !content.includes('JavaScript cho PWA')) {
      content = content.replace('</head>', pwaJS + '\n</head>');
    }
    
    // Thêm orientation warning sau <body>
    if (content.includes('<body>') && !content.includes('orientation-warning')) {
...
filesToUpdate.forEach(addPWAToFile);

console.log('🎉 PWA meta tags added to all files!');